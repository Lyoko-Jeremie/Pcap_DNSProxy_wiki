...
* 确定工具文件夹的名称和路径后进入文件夹内，运行 `ServiceInstall.bat`
    ...
    * **注意 Windows 8.1 下批处理可能无法找到程序，也可直接运行 Pcap_DNSProxy 程序进行Windows防火墙测试**<br />
...
* `Data` - 数据区域
    ...
    * `ICMP PaddingData` - ICMP附加数据，Ping程序发送请求时为补足数据使其达到Ethernet类型网络最低的可发送长度时添加的数据：长度介乎于18字节-64字节ASCII数据之间，留空则使用 Microsoft Windows Ping(Windows)/Linux Ping(Linux) 程序的ICMP附加数据，默认为空
